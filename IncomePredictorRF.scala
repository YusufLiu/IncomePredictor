...
object LRIncomePredictor{
    ...
    def IncomePredictor(source1: String, source2 : String):Integer={
        ...
        .load("file:///notebook/sample_libsvm_data.txt")

        val vector_data = income_data_vector.select(col("features"),col("ING_MON_SB").alias("label"))
        val test_datatest_data  = test_data_vector.select(col("features"),col("ING_MON_SB").alias("label"))
        ...
        val model = pipeline.fit(vector_data)
        ...
        prediction10.select(col("accurate")).where(col("accurate")===1).count()
